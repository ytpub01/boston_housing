...
{\color{incolor}In [{\color{incolor}2}]:} \PY{c+c1}{\PYZsh{} TODO: Minimum price of the data}
        ...
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{Statistics for sample Boston housing dataset:}\PY{l+s+se}{\PYZbs{}n}\PY{l+s+s2}{\PYZdq{}}\PY{p}{)}
        ...
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{Standard deviation of prices (with 1 degree of freedom): \PYZdl{}}\PY{l+s+si}{\PYZob{}:,.2f\PYZcb{}}\PY{l+s+s2}{\PYZdq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{std\PYZus{}price}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
...
{\color{incolor}In [{\color{incolor}4}]:} \PY{c+c1}{\PYZsh{} Calculate the performance of this model}
        ...
        \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{Model has a coefficient of determination, R2, of }\PY{l+s+si}{\PYZob{}:.3f\PYZcb{}}\PY{l+s+s2}{.}\PY{l+s+s2}{\PYZdq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{score}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
...
\textbf{Hint:} The R2 score is the proportion of the variance in the
...
\textbf{The coefficient of determination R2 = 0.93 is very close to 1,
indicating high correlation between the predicted and actual values, so
that this model has successfully captured the variation of the target
variable.}
...
{\color{incolor}In [{\color{incolor}5}]:} \PY{c+c1}{\PYZsh{} TODO: Import \PYZsq{}train\PYZus{}test\PYZus{}split\PYZsq{}}
        ...
        \PY{n}{X\PYZus{}train}\PY{p}{,} \PY{n}{X\PYZus{}test}\PY{p}{,} \PY{n}{y\PYZus{}train}\PY{p}{,} \PY{n}{y\PYZus{}test} \PY{o}{=} \PY{n}{train\PYZus{}test\PYZus{}split}\PY{p}{(}\PY{n}{features}\PY{p}{,} \PY{n}{prices}\PY{p}{,} \PY{n}{random\PYZus{}state}\PY{o}{=}\PY{l+m+mi}{17}\PY{p}{)}
...
\textbf{The model with maximum depth of 3 performs best among the models
presented. It shows both the training and testing curves converging to
the same score, indicating that the model is not overfitting. Also, it
is not underfitting since the convergence score is relatively high, at
about 0.8, as compared with the model of a depth of 1 which gives a
lower score.}

\textbf{In each of the learning graphs, the training and testing curves
are approaching each other (i.e.~the training and testing scores are
converging) as we use more training points. This is happening well
before all training points are used, so adding more training points is
not useful.}
...
\textbf{Hint:} The answer comes from the output of the code snipped
above.
...
\textbf{The predicted selling prices for clients 1, 2 and 3 are,
respectively, \textbackslash{}\$409,419.23, \textbackslash{}\$237,814.29
and \textbackslash{}\$1,021,650.00. These prices seem reasonable given
that client 3 has a house with feature values associated with the
highest prices: high total number of rooms, low poverty level in the
area and low student-to-teacher ratio. Client 2 is has by contrast the
lowest number of rooms, a high poverty rate and a high
student-to-teacher ratio. Client 1's values are in-between, with a price
to match.}
